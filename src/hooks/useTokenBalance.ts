...
import { getRavAddress, getRshareAddress, getRbondAddress, getRavNativeLPAddress, getRshareNativeLPAddress } from 'utils/addressHelpers'
...
export const useTokenBalanceOfContract = (tokenAddress: string, contractAddress: string) => {
  const [balance, setBalance] = useState(new BigNumber(0))
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      const res = await getTokenBalance(ethereum, tokenAddress, contractAddress)
      setBalance(new BigNumber(res))
    }
  ...
  }, [contractAddress, ethereum, tokenAddress, slowRefresh])
