...
import { getTokenBalance, getTokenBalanceWithoutDecimals } from "utils/erc20";
import {
  getRavAddress,
  getRshareAddress,
  getRbondAddress,
...
export const useTokenBalanceWithoutDecimals = (tokenAddress: string) => {
  const [balance, setBalance] = useState(new BigNumber(0));
  const { account, ethereum }: { account: string; ethereum: provider } =
    useWallet();
  const { fastRefresh } = useRefresh();
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      const res = await getTokenBalanceWithoutDecimals(
        ethereum,
        tokenAddress,
        account
      );
