...
const WithdrawCard: React.FC<Props> = ({ masonry, period }) => {
  const { userData, withdrawLockupEpochs, epoch, nextEpochPoint } = masonry

  const currentTimeMilis = useCurrentTime()

  const lNow = DateTime.fromMillis(currentTimeMilis).setZone('utc')
  const targetTimeMilis = new BigNumber(userData?.epochTimerStart).plus(new BigNumber(withdrawLockupEpochs)).minus(new BigNumber(epoch)).minus(1).times(new BigNumber(period)).plus(new BigNumber(nextEpochPoint)).times(1000)
