...
const HexCard: React.FC<CardProps> = ({ tokenAddress, tokenName, tokenPriceUSD, adaPrice, marketCap, circSupply, totalSupply }) => {
  ...
  return (
    <Card>
      ...
      <Text color='#007ABE'>{/* tokenPriceInAda.toFormat(4) */ 0.0000} ADA</Text>
      <Text color='#9D9D9D' fontSize='11px'>${/* tokenPriceUSD.toFormat(4) */ 0.0000}</Text>
      <Text color='#9D9D9D' fontSize='12px'><b>Market Cap:</b> ${/* marketCap.div(1e18).toFormat(2) */ 0.00}</Text>
      <Text color='#9D9D9D' fontSize='12px'><b>Circulating Supply:</b> {/* circSupply.div(1e18).toFormat(0) */ 0.00}</Text>
      <Text color='#9D9D9D' fontSize='12px'><b>Total Supply:</b> {/* totalSupply?.div(1e18).toFormat(0) */ 0.00}</Text>
    </Card>
  )
}
