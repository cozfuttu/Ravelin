...
const HealthBarContainer = styled.div<ContainerProps>`
  ...
  justify-content: ${(props) =>
    props.type === "one" ? "flex-start" : "flex-end"};
  border: ridge;
  border-width: 5px;
  border-radius: 12px;
  overflow: hidden;
...
const Health = styled.div<HealthProps>`
  ...
  animation-delay: ${(props) =>
    props.attackStyle === "heal" ? "1.1s" : "0.65s"};
`;
...
export const HealthStatus: React.FC<CombatTextProps> = ({
  health = 100,
  hunterName,
  attackStyle,
}) => {
...
export const BugCountText: React.FC<BugCountTextProps> = ({
  bugCount = 0,
  attackStyle,
}) => {
  ...
  return (
    ...
    >
      <Text mr="0.5rem" mb="12px">
        Total Bugs Killed:
      </Text>
      <BugCountTextAnimated
...
export const CombatCounter: React.FC<{
  initialTime: number;
  isDead: boolean;
}> = ({ initialTime, isDead }) => {
  ...
  return (
    ...
    >
      <ClockImage src="/images/hunter/missionIcons/clock.png" />
      {isDead ? (
        <DeadImage src="/images/hunter/missionIcons/dead.png" />
      ) : (
